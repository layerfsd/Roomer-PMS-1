...
{$ENDIF}
  public
    ...
    procedure PaintBG(BGBmp: TBitmap; R: TRect; const ci: TCacheInfo);
    procedure ShowHint(TheControl: TControl; const HintText: String);
    procedure HideHint;
...
{$ENDIF}
  end {$IFDEF DELPHI_XE2}deprecated 'Use TacAlphaHints instead'{$ENDIF};
...
{$IFNDEF NOTFORHELP}
  ...
  public
    ...
    procedure PaintBG(Bmp: TBitmap; aRect: TRect);
...
var
  ...
  X, Y, B, R: Integer;
begin
  if Src1.Width < WidthOf(aRect) then
    Exit;

  if Src1.Height < HeightOf(aRect) then
    Exit;
  ...
  for Y := aRect.Top to B do begin
    ...
    S2 := Src2.ScanLine[Y];
    M  := MskBmp.ScanLine[Y];
    for X := aRect.Left to R do
      with S2[X] do begin
        S1[X].R := ((S1[X].R - R) * M[X].R + R shl 8) shr 8;
        S1[X].G := ((S1[X].G - G) * M[X].G + G shl 8) shr 8;
        S1[X].B := ((S1[X].B - B) * M[X].B + B shl 8) shr 8;
      end
  end;
end;
...
procedure BorderByMask(SrcBmp, MskBmp: TBitmap; const ColorTop, ColorBottom: TsColor);
var
  S1, S2, S2t, S2b: PRGBAArray;
  t, b: boolean;
  X, Y, sw, sh: Integer;

  function BlackPoint(const c: TsColor_): boolean;
  begin
...
procedure TsHintManager.PaintBG(BGBmp: TBitmap; R: TRect; const ci: TCacheInfo);
var
  aRect: TRect;
  ...
  procedure FillCanvas(bmp: TBitmap);
  begin
  ...
  begin
    ...
    if (HintKind.GradientPercent > 0) then begin
      if iDrawed then begin
        ...
        try
          if Length(HintKind.GradientData) > 0 then
            PaintGrad(Bmp, Rect(0, 0, Bmp.Width, Bmp.Height), GradientArray)
          else
            FillCanvas(Bmp);
...
begin
  if not acHintsInEditor then
    Transparency := 0
  else
    Transparency := HintKind.Transparency;
  ...
  if ci.Ready and (Transparency = 100) then begin
    ...
    if not ci.Ready or (Transparency = 0) then begin
      PaintAddons(BGBmp);
    end
    else
...
procedure TsHintManager.ShowHint(TheControl: TControl; const HintText: String); // Added by Matthew Bieschke
Var
...
begin
  ...
  If not Assigned(FTempHint) Then
    Case HintKind.Style of
      hsBalloon:
  ...
  If Assigned(FTempHint) Then
    With FTempHint Do Begin
      ...
      if Assigned(OnShowHint) then begin
        ...
        NewText := HintText;
        OnShowHint(NewText, b, HintInfo, F);
        if HintText <> NewText then
          HintRect := CalcHintRect(HintKind.MaxWidth, NewText, NIL);
...
begin
  case FPredefinitions of
    ...
    shStandard:
      HintKind.Style := hsStandard;

    shNone:
      HintKind.Style := hsNone;
  end;
end;
...
{$IFNDEF ACHINTS}
  ...
  if Result then
    Result := (Manager.SkinSection <> '') and (DefaultManager.GetSkinIndex(Manager.SkinSection) > -1);
{$ELSE}
  Result := False
{$ENDIF}
end;
...
begin
  if not Assigned(Manager) or (Manager.HintKind.Style = hsNone) then
    exit;

  Shown := IsWindowVisible(Handle);

  if Manager.Skinned and acLayered and not ((DefaultManager.gd[SkinIndex].Transparency = 100) and (BorderIndex > -1)) then
  ...
  Caption := AHint;
  if (FHintLocation.X = 0) or (FHintLocation.Y = 0) then
    p := GetMousePosition
  else
    p := FHintLocation;
  ...
  if FMousePos in [mpLeftBottom, mpRightBottom] then
    OffsetRect(Rect, 0, - h);

  if FMousePos in [mpRightTop, mpRightBottom] then
    OffsetRect(Rect, -w, 0);
...
{$IFDEF DELPHI6UP}
  ...
  if Rect.Bottom > Monitor.Top + Monitor.Height then begin
    if (Manager.HintKind.Style in [hsComics, hsEllipse, hsBalloon]) then begin
      ...
      Rect.Top := Monitor.Top + Monitor.Height - h;
      
    t := False;
    Auto := True
  end;
  if Rect.Top < Monitor.Top then begin
  ...
  if Rect.Right > Monitor.Left + Monitor.Width then begin
    ...
    if (Manager.HintKind.Style in [hsComics, hsEllipse, hsBalloon]) and (FMousePos = mpLeftTop) then begin
      ...
      Rect.Left := Monitor.Left + Monitor.Width - w;
  end;
  if Rect.Left < Monitor.Left then begin
...
{$ENDIF}
    ...
    DC := GetDC(0); // grabbing
    if DC = 0 then begin
      {$IFNDEF ACHINTS}ShowError('GDI error (out of resources)');{$ENDIF}
      Exit
    end;
    if not Assigned(ScreenBmp) then
      ScreenBmp := CreateBmp32(w, h)
    else begin
      ScreenBmp.Width := w;
      ScreenBmp.Height := h
    end;
    ...
    ReleaseDC(0, DC);
    if Assigned(ScreenBmp) then
      BitBlt(Manager.FCacheBmp.Canvas.Handle, 0, 0, w, h, ScreenBmp.Canvas.Handle, 0, 0, SrcCopy);
...
{$ENDIF}
      ...
      if Manager.FHTMLMode then begin
        ...
        acDrawText(Manager.FCacheBmp.Canvas.Handle, PacChar(HintText), Result,
                   DT_CALCRECT or DT_CENTER or DT_VCENTER or DT_WORDBREAK or DT_NOPREFIX or DrawTextBiDiModeFlagsReadingOnly);

      if ((Manager.HintKind.Style = hsBalloon) or (Manager.HintKind.Style = hsEllipse)) and (WidthOf(Result) < 50) then
...
begin
  if Assigned(Manager) then
    with Manager do begin
      if HintKind.ShadowEnabled then
        PaintShadow;
      ...
      try
        ...
        if Assigned(MaskBmp) and Assigned(BodyBmp) then
          SumByMask(FCacheBmp, BodyBmp, MaskBmp, ClientRect);
...
begin
  ...
  {$IFNDEF ACHINTS}
      if Manager.Skinned then
        Bmp.Canvas.Font.Color := DefaultManager.gd[SkinIndex].Fontcolor[1];
  {$ENDIF}
      sHTML := TsHtml.Create;
...
{$IFDEF TNTUNICODE}
        if TntApplication <> nil then
          acWriteTextEx(BMP.Canvas, PacChar(TntApplication.Hint), True, R, Flags, SkinIndex, False, DefaultManager);
{$ELSE}
...
{$IFDEF TNTUNICODE}
        if TntApplication <> nil then
          acDrawText(BMP.Canvas.Handle, PacChar(TntApplication.Hint), R, DT_CENTER or DT_NOPREFIX or DT_WORDBREAK or DrawTextBiDiModeFlagsReadingOnly);
{$ELSE}
...
begin
  if acHintsInEditor then
    Result := Manager.HintKind.ShadowTransparency
  else begin
...
{$IFNDEF ACHINTS}
  ...
  for y := 0 to h do
    for x := 0 to w do
      if GetAPixel(Result, x, y).I = CI.FillColor then
        SetAPixel(Result, x, y, White)
      else
        SetAPixel(Result, x, y, Black);
{$ENDIF}
end;
...
{    if DefaultManager.ma[BorderIndex].BorderWidth > 0 then begin
    ...
    case Border of
      0: begin
        if DefaultManager.ma[BorderIndex].WL > 0 then
          Result := DefaultManager.ma[BorderIndex].WL
        else
          Result := WidthOf(DefaultManager.ma[BorderIndex].R) div (DefaultManager.ma[BorderIndex].ImageCount * 3)
      end;

      1: begin
        if DefaultManager.ma[BorderIndex].WT > 0 then
          Result := DefaultManager.ma[BorderIndex].WT
        else
          Result := HeightOf(DefaultManager.ma[BorderIndex].R) div ((DefaultManager.ma[BorderIndex].MaskType + 1) * 3)
      end;

      2: begin
        if DefaultManager.ma[BorderIndex].WR > 0 then
          Result := DefaultManager.ma[BorderIndex].WR
        else
          Result := WidthOf(DefaultManager.ma[BorderIndex].R) div (DefaultManager.ma[BorderIndex].ImageCount * 3)
      end

      else {3} begin
        if DefaultManager.ma[BorderIndex].WB > 0 then
          Result := DefaultManager.ma[BorderIndex].WB
        else
          Result := HeightOf(DefaultManager.ma[BorderIndex].R) div ((DefaultManager.ma[BorderIndex].MaskType + 1) * 3)
      end;
    end
...
begin
  if Manager.Skinned then
    ...
    R := MainRect;
    if not acLayered or acHintsInEditor then
      tr := Manager.HintKind.Transparency * integer(Manager.HintKind.Transparency > 0)
    else
      tr := 0;
  ...
  if Manager.Skinned then
    ...
    try
      ...
      for i := 0 to Manager.HintKind.ShadowBlur do begin
        ...
        tr := WidthOf(R) shr 3 {div 8} - Round(i * k * 2);
        if tr < 0 then tr := 0;
        case FMousePos of
          mpLeftTop, mpRightTop: begin
...
function TsBalloonHintWindow.CalcHintRect(MaxWidth: Integer; const AHint: string; AData: Pointer): TRect;
var
  s, l: real;
begin
...
procedure TsHintKind.SetBlur(const Value: integer);
begin
  //
end;
...
procedure TsHintKind.SetShadowColor(const Value: TColor);
begin
  //
end;
