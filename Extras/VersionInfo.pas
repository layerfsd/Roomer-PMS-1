...
type
  ...
  TVersionNumber = record
    ...
    class operator Implicit(Ver: TVersionNumber): string;
      {Operator overload that performs implicit conversion of TVersionNumber
...
function VerNumToStr(const Ver: TVersionNumber): string;
...
begin
  RegisterComponents('Roomer', [TVersionInfo]);
end;
...
function TVersionInfo.GetCharSetCode: WORD;
...
function TVersionInfo.GetCurrentTranslation: Integer;
...
function TVersionInfo.GetFixedFileInfoItemByIdx(Index: Integer): DWORD;
...
function TVersionInfo.GetLanguageCode: WORD;
...
function TVersionInfo.GetStringFileInfo(const Name: string): string;
...
function TVersionInfo.GetStringFileInfoByIdx(Index: Integer): string;
...
procedure TVersionInfo.GetTransBuffer(Len: UINT);
  {Creates a translation table buffer of required size.
    @param Required buffer size in bytes.
  }
begin
...
function TVersionInfo.GetTransStr: string;
  {Encodes information about the current translation in a string.
    @return Required translation information.
  }
var
...
procedure TVersionInfo.SetCurrentTranslation(const Value: Integer);
...
class operator TVersionNumber.GreaterThan(Ver1,
