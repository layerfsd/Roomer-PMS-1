...
procedure InvoiceLineHolderRecToDxMemData(rec: recInvoiceLineHolder; dxMemData: TDxMemData);
procedure DxMemDataToInvoiceLineHolderRec(dxMemData: TDxMemData; var rec: recInvoiceLineHolder);
...
begin
  ...
  begin
    ...
    dxMemData['PurchaseDate'] := dateToSqlString(PurchaseDate);
    dxMemData['AutoGen'] := AutoGen;

    dxMemData['ItemId'] := ItemID;
    dxMemData['Description'] := Description;
    dxMemData['Number'] := Number;
    dxMemData['Price'] := Price;
    dxMemData['Total'] := Total;
    dxMemData['VATType'] := VATType;
    dxMemData['ItemNumber'] := ItemNumber;
    dxMemData['TotalWOVat'] := TotalWOVAT;
    dxMemData['Vat'] := VAT;
    dxMemData['AutoGenerated'] := AutoGenerated;
    dxMemData['CurrencyRate'] := CurrencyRate;
    dxMemData['Currency'] := Currency;
    dxMemData['ReportDate'] := dateToSqlString(ReportDate);
    dxMemData['ReportTime'] := ReportTime;
    dxMemData['Persons'] := Persons;
    dxMemData['Nights'] := Nights;
    dxMemData['BreakfastPrice'] := BreakfastPrice;
    dxMemData['AYear'] := Ayear;
    dxMemData['AMon'] := Amon;
    dxMemData['ADay'] := Aday;

    dxMemData['ItemCurrencyRate'] := ItemCurrencyRate;
    dxMemData['ItemCurrency'] := ItemCurrency;

    dxMemData['ilAccountKey'] := ilAccountKey;
    dxMemData['ImportRefrence'] := ImportRefrence;
    dxMemData['ImportSource'] := ImportSource;
    dxMemData['isPackage'] := IsPackage;
    dxMemData['Discount'] := Discount;
    dxMemData['Discount_isPrecent'] := Discount_isPrecent;

    dxMemData['confirmDate'] := confirmDate;
    dxMemData['confirmAmount'] := confirmAmount;

    dxMemData['InvoiceIndex'] := InvoiceIndex;
  end;
end;
...
begin
  ...
  begin
    ...
    PurchaseDate := SqlStringToDate(dxMemData['PurchaseDate']);
    AutoGen := dxMemData['AutoGen'];

    ItemID := dxMemData['ItemId'];
    Description := dxMemData['Description'];
    Number := dxMemData['Number'];
    Price := dxMemData['Price'];
    Total := dxMemData['Total'];
    VATType := dxMemData['VATType'];
    ItemNumber := dxMemData['ItemNumber'];
    TotalWOVAT := dxMemData['TotalWOVat'];
    VAT := dxMemData['Vat'];
    AutoGenerated := dxMemData['AutoGenerated'];
    CurrencyRate := dxMemData['CurrencyRate'];
    Currency := dxMemData['Currency'];
    try
      ReportDate := SqlStringToDate(dxMemData['ReportDate']);
    Except
      ReportDate := 0
    End;
    ReportTime := dxMemData['ReportTime'];
    Persons := dxMemData['Persons'];
    Nights := dxMemData['Nights'];
    BreakfastPrice := dxMemData['BreakfastPrice'];
    Ayear := dxMemData['AYear'];
    Amon := dxMemData['AMon'];
    Aday := dxMemData['ADay'];

    ItemCurrencyRate := dxMemData['ItemCurrencyRate'];
    ItemCurrency := dxMemData['ItemCurrency'];

    ilAccountKey := dxMemData['ilAccountKey'];
    ImportRefrence := dxMemData['ImportRefrence'];
    ImportSource := dxMemData['ImportSource'];
    IsPackage := dxMemData['isPackage'];
    Discount := dxMemData['Discount'];
    Discount_isPrecent := dxMemData['Discount_isPrecent'];
    confirmDate := dxMemData['confirmDate'];
    confirmAmount := dxMemData['confirmAmount'];
    InvoiceIndex := dxMemData['InvoiceIndex'];
  end;
end;
