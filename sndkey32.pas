...
const MyWait : boolean = false;

Function SendKeys(SendKeysString : PChar; Wait : Boolean) : Boolean;
function AppActivate(WindowName : PChar) : boolean;

Procedure MySendKeyDown(VKey: Byte; NumTimes : Word; GenUpMsg : Boolean);
Procedure MySendKeyUp(VKey: Byte);
...
Procedure MyKeyboardEvent(VKey, ScanCode : Byte; Flags : Longint);
var
  KeyboardMsg : TMsg;
begin
  keybd_event(VKey, ScanCode, Flags,0);
  If (MyWait) then While (PeekMessage(KeyboardMsg,0,WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)) do begin
...
begin
  ...
  For Cnt:=1 to NumTimes do
    If (VKey in ExtendedVKeys )then begin
      MyKeyboardEvent(VKey, ScanCode, KEYEVENTF_EXTENDEDKEY);
      If (GenUpMsg) then
        MyKeyboardEvent(VKey, ScanCode, KEYEVENTF_EXTENDEDKEY or KEYEVENTF_KEYUP)
    end else begin
      MyKeyboardEvent(VKey, ScanCode, 0);
      If (GenUpMsg) then MyKeyboardEvent(VKey, ScanCode, KEYEVENTF_KEYUP);
    end;
end;
...
begin
 ...
 //   MyKeyboardEvent(VKey, ScanCode, KEYEVENTF_EXTENDEDKEY and KEYEVENTF_KEYUP)
   MyKeyboardEvent(VKey, ScanCode, KEYEVENTF_KEYUP)
  else
   MyKeyboardEvent(VKey, ScanCode, KEYEVENTF_KEYUP);
end;
