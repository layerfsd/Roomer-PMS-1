...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Grids, ExtCtrls, ComCtrls, StdCtrls, ImgList, Menus, Buttons, Data.DB,
  Data.Win.ADODB

    , uDateUtils, ActnList, System.Actions, Generics.Collections, variants, cmpRoomerDataSet
    ...
    , sPanel, sEdit, sLabel, sGroupBox, sStatusBar, sButton, sSkinProvider, sPageControl

    , cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters, cxStyles, dxSkinsCore, dxSkinDarkSide, dxSkinDevExpressDarkStyle, dxSkinMcSkin,
  dxSkinOffice2013White, dxSkinsDefaultPainters, dxSkinscxPCPainter, cxCustomData, cxFilter, cxData, cxDataStorage, cxEdit, cxNavigator, cxDBData, cxMemo,
  cxButtonEdit, cxSpinEdit, cxCalc, cxContainer

    , frxExportMail, frxExportImage, frxExportRTF, frxExportHTML, frxClass, frxExportPDF, frxDesgn, frxDBSet

    , cxPropertiesStore, sCheckBox, cxTextEdit, cxMaskEdit, cxDropDownEdit, cxGridLevel, cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxClasses,
  cxGridCustomView, cxGrid

    , dxmdaset, dxSkinCaramel, dxSkinCoffee, dxSkinTheAsphaltWorld, dxSkinBlack, dxSkinBlue, dxSkinBlueprint, dxSkinDarkRoom,
...
{$M+}
  ...
  TfrmInvoice = class(TForm)
    ...
    procedure agrLinesSelectCell(Sender: TObject; ACol, ARow: integer; var CanSelect: boolean);
    procedure agrLinesSetEditText(Sender: TObject; ACol, ARow: integer; const Value: string);
    ...
    procedure agrLinesGetCellColor(Sender: TObject; ARow, ACol: integer; AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
    ...
    procedure agrLinesGetAlignment(Sender: TObject; ARow, ACol: integer; var HAlign: TAlignment; var VAlign: TVAlignment);
  ...
  private
    ...
    function UpdateCurrencyRoomPrice(RoomReservation: integer; Currency: string; CurrencyRate: Double; convert: Double): boolean;
    ...
    function AddLine(sItem, sText: string; iNumber: Double; FPrice, FTotal: Double; PurchaseDate: TDate; bAuto: boolean; Refrence, Source: string;
      isPackage: boolean; noGuests: integer; confirmDate: TDateTime; confirmAmount: Double; rrAlias: integer; AutoGen : string; itemIndex: integer = 0): integer;

    Function AddRoom(Room : String; fRoomPrice: Double; FromDate: TDate; ToDate: TDate; dayCount: integer; sText: string; bGetGuestName: boolean; RoomReservation: integer;
      DiscountAmount: Double; DiscountIsPercentage: boolean; DiscountText: string; GuestName: String; NumGuests: integer; isPackage: boolean;
      rrAlias: integer): integer;

    Procedure AddRoomTax(totalTax: Double; TaxUnits: Double; taxItem: String = ''; iAddAt: integer = 0);
    ...
    procedure SaveHeader(FTotal, fVat, fWOVat: Double; ExecutionPlan: TRoomerExecutionPlan);
    ...
    function GetInvoiceHeader(Res, RoomRes: integer; arSet: TdxMemData): boolean; overload;
    ...
    procedure AddDeleteFromInvoiceToExecutionPlan(ExecutionPlan: TRoomerExecutionPlan);

    function LocateDate(recordSet: TRoomerDataset; field: String; Value: TDateTime): boolean;
    ...
    procedure AddPaymentToOfflineData(theData: recPaymentHolder);
    function GetItemIvoiceLinesForTaxCalculations: TList<TInvoiceItemEntity>;
    procedure RemoveTempInvoiceItemList(_tempInvoiceItemList: TList<TInvoiceItemEntity> = nil);
    function CheckIfWithdrawlAllowed(removeIfNotAllowed: boolean; Editing: boolean): boolean;
    function CheckIfWithdrawlAllowed_X(Editing: boolean; Value: String): boolean;
...
procedure EditInvoice(reservation, RoomReservation, SplitNumber: integer; Arrival, Departure: TDate; bCredit, aModal: boolean; fakeGroup: boolean);
...
procedure TfrmInvoice.AddDeleteFromInvoiceToExecutionPlan(ExecutionPlan: TRoomerExecutionPlan);
var
  s: string;
begin
  s := '';
...
function TfrmInvoice.AddLine(sItem, sText: string; iNumber: Double; // -96
  FPrice, FTotal: Double; PurchaseDate: TDate; bAuto: boolean; Refrence, Source: string; isPackage: boolean; noGuests: integer; confirmDate: TDateTime;
  confirmAmount: Double; rrAlias: integer; AutoGen : string; itemIndex: integer = 0): integer;
var
...
begin
  ...
  agrLines.Cells[col_ItemCount, iRow] := _floattostr(invoiceLine.FNumber, vWidth, vDec);
  agrLines.Cells[col_ItemPrice, iRow] := _floattostr(invoiceLine.FPrice, vWidth, vDec);
  agrLines.Cells[col_TotalPrice, iRow] := _floattostr(invoiceLine.FTotal, vWidth, vDec);
  ...
  agrLines.Cells[col_confirmdate, iRow] := _dbDateAndTime(invoiceLine.FConfirmDate, false);
            ...
            begin
              ...
              begin
                ...
                dVat := GetVATForItem(itemId, itemAmount, _StrToFloat(agrLines.Cells[col_ItemCount, i]),
                  TInvoiceRoomEntity.create(itemId, 1, _StrToFloat(agrLines.Cells[col_ItemCount, i]), itemAmount, 0), tempInvoiceItemList,
                  ItemTypeInfo,
                  edtCustomer.Text);
...
//                tempInvoiceItemList := GetItemIvoiceLinesForTaxCalculations;
//                try
                  dVat := zCurrencyRate * GetVATForItem(itemId, itemAmount, _StrToFloat(agrLines.Cells[col_ItemCount, i]),
                    TInvoiceRoomEntity.create(itemId, 1, _StrToFloat(agrLines.Cells[col_ItemCount, i]), itemAmount, 0), tempInvoiceItemList,
                    ItemTypeInfo,
                    edtCustomer.Text);
//                finally
...
//                end;
                ...
                // end;
                // Formula
                // dWork := nativeTaxAmount / (1 + (ItemTypeInfo.VATPercentage / 100));
                // dVat := nativeTaxAmount - dWork;
              end
              else
              begin
                try
    ...
    begin
      ...
      try
        dVat := GetVATForItem(g.qRoomRentItem, nativeRent, ttRentNumber, // 1,
                TInvoiceRoomEntity.create(g.qRoomRentItem, 1, 1, nativeRent, 0), tempInvoiceItemList,
                ItemTypeInfo,
                edtCustomer.Text);
      finally
        RemoveTempInvoiceItemList;
      ...
      // Formula
      // dWork := (nativeRent) / (1 + (ItemTypeInfo.VATPercentage / 100));
      // dVat := (nativeRent) - dWork;
      rentVat := dVat;
    end;
    // end;
    ...
    edtTotal.Text := _floattostr((nativeTotal) - ttVAT, vWidth, vDec);
    ...
    edtDownPayments.Text := _floattostr(TotalDownPayments, vWidth, vDec);
    edtBalance.Text := _floattostr(TotalBalance, vWidth, vDec);

    if (edtCurrency.Text <> '') and (edtCurrency.Text <> zNativeCurrency) then
      edtForeignCurrency.Text := _floattostr((nativeTotal) / GetRate(edtCurrency.Text), vWidth, vDec);
...
begin
  ...
  begin
    d.GetBackupPersons(FRoomReservation, MemData3);
    MemData3.first;
    while NOT MemData3.eof do
    begin
      if MemData3['MainName'] = 1 then
      begin
        edtRoomGuest.Text := MemData3['Name'];
      end;
      MemData3.Next;
    end;
...
procedure TfrmInvoice.AddRoomTax(totalTax: Double; TaxUnits: Double; taxItem: String = ''; iAddAt: integer = 0);
var
...
begin
  ...
  try
    ...
    idx := AddLine(Item, aText, TaxUnits, unitPrice, totalTax, 0, true, '', '', false, 0, confirmDate, confirmAmount, -1,''); //77ATH
    ...
    try
      ...
      begin
        ...
        begin
          ...
          confirmAmount := rSet.GetFloatValue(rSet.FieldByName('ConfirmAmount'));
...
function TfrmInvoice.AddRoom(Room : String; fRoomPrice: Double; FromDate: TDate; ToDate: TDate; dayCount: integer; sText: string; bGetGuestName: boolean;
  RoomReservation: integer; DiscountAmount: Double; DiscountIsPercentage: boolean; DiscountText: string; GuestName: String; NumGuests: integer;
  isPackage: boolean; rrAlias: integer): integer;
var
  i, idx, iCol: integer;
...
begin
  ...
  idx := AddLine(RmRntItem, sText, dayCount, fRoomPrice, fRoomPrice * dayCount, 0, true, '', '', isPackage, iNumGuests, confirmDate, confirmAmount, rrAlias,''); //*77
  ...
  begin
    ...
    idx := AddLine(DiscountItem, DiscountText, dayCount, fWork, fWork * dayCount, 0, true, '', '', false, NumGuests, confirmDate, confirmAmount, rrAlias,'');
...
procedure TfrmInvoice.ApplyRateToOther(RoomReservation: integer; RoomType: string);
var
  reservation: integer;
  RoomNumber: string;
  RateDate: TDateTime;
...
//  mRoomRatesTmp.Filtered := true;
  ...
  try
    ...
    begin
      ...
      if (mRoomRes.FieldByName('RoomReservation').asinteger <> RoomReservation) AND (mRoomRes.FieldByName('Arrival').asdateTime = Arrival) AND
...
procedure TfrmInvoice.ApplyNettoRateToNullPrice(NewRate: Double; RoomReservation: integer; RoomType: string);
var
  currentRoomReservation: integer;
begin
  mRoomRates.DisableControls;
...
function TfrmInvoice.CalcOnePrice(RoomReservation: integer; NewRate: Double = 0): boolean;
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          Rate := GetDayRate(RoomType, room, aDate, Guests, ChildrenCount, infantCount, Currency, priceID, Discount, ShowDiscount, isPercentage, isPaid, false);
        end;
...
procedure TfrmInvoice.tvRoomResAvragePricePropertiesEditValueChanged(Sender: TObject);
...
procedure TfrmInvoice.tvRoomResChildrenCountPropertiesEditValueChanged(Sender: TObject);
var
  RoomReservation: integer;
  oldValue: integer;
  newValue: integer;
...
procedure TfrmInvoice.tvRoomResColumn1PropertiesButtonClick(Sender: TObject; AButtonIndex: integer);
var
  RoomReservation: integer;
begin
  RoomReservation := mRoomRes.FieldByName('roomreservation').asinteger;
...
procedure TfrmInvoice.tvRoomResinfantCountPropertiesEditValueChanged(Sender: TObject);
var
  RoomReservation: integer;
  oldValue: integer;
  newValue: integer;
...
begin
  ...
  try
    ...
    theData.Currency := zCurrenctCurrency; // edtCurrency.Text;
...
begin
  ...
  begin
    ...
    begin
      iNights := trunc(TRoomInfo(agrLines.Objects[2, i]).FTo) - trunc(TRoomInfo(agrLines.Objects[2, i]).FFrom);
    end;
    ...
    iRRAlias := -1;
    try
      if lineRRalias <> '' then iRRAlias := strToInt(lineRRalias);
    Except
    end;
    ...
    try
      sTmp := lineIsPackage;
      if sTmp = 'Yes' then isPackage := true;
    except
    end;
    ...
    m.FieldByName('PurchaseDate').AsString     := _DateToDbDate(integer(agrLines.Objects[1, i]), true);
    ...
    m.FieldByName('TotalWOVat').asFloat        := TotalPrice-fVat;  // _CommaToDot(floattostr(iMultiplier * fItemTotalWOVat));
    m.FieldByName('VAT').asFloat               := fVat;  // _CommaToDot(floattostr(iMultiplier * fItemTotalVAT));
...
begin
  ...
  Again:
    ...
    begin
      zS := '';
      sql := 'SELECT * FROM invoiceheads where Reservation = %d '#10 + '   and RoomReservation = %d and SplitNumber = %d '#10 +
        '   and InvoiceNumber = -1 and Finished = 0';
      if zrSet.active then
        zrSet.Close;

      zS := format(sql, [publicReservation, FRoomReservation, FnewSplitNumber]);
...
{$ENDREGION}
      ...
      begin
        ...
        sql := ' SELECT r.* FROM  reservations r ' + ' WHERE r.Reservation = %d ';
    ...
    begin
      ...
      d.GetBackupInvoiceHeadsReservationRoomReservationsSplitNumber(publicReservation, FRoomReservation, FnewSplitNumber, MemData2);
      MemData2.first;
      if not MemData2.eof then
      begin
        // Sækja Invoice sem er til
        zInvoiceNumber := MemData2.FieldByName('InvoiceNumber').asinteger;

        if FnewSplitNumber <> 2 then // ef ekki staðgreiðslureikningur
        begin
{$REGION 'Set Invoice type'}
          if MemData2.FieldByName('InvoiceType').asinteger = 4 then // Free text
          begin
            GetInvoiceHeader(publicReservation, FRoomReservation, MemData2);
            rgrInvoiceType.itemIndex := 4;
          end
          else if MemData2.FieldByName('InvoiceType').asinteger = 1 then
          // Reservation customer
          begin
            GetReservationHeader(publicReservation, FRoomReservation);
            rgrInvoiceType.itemIndex := 1;
          ...
          begin
            rgrInvoiceType.itemIndex := MemData2.FieldByName('InvoiceType').asinteger;
          end;
...
{$ENDREGION}
      ...
      begin
        ...
        zrSet := d.roomerMainDataSet.ActivateNewDataset(d.GetBackupReservations(publicReservation));
    ...
    zS := select_Invoice_LoadInvoice3(FRoomReservation);
    ...
    begin
      zRoomRSet := d.roomerMainDataSet.ActivateNewDataset(d.GetBackupReservationRoomReservations(publicReservation));
    end;
    ...
    zCurrenctCurrency := edtCurrency.Text;
...
//           debugmessage(s);
      ...
      if d.roomerMainDataSet.OfflineMode OR ExecutionPlan.Execute(ptQuery) then
      begin
        zRoomRSet.first;
        index := 0;
        ...
        begin
          ...
          if NOT d.roomerMainDataSet.OfflineMode then
            ...
            rSet := d.roomerMainDataSet.ActivateNewDataset(d.GetBackupRoomReservationRoomsDate(RoomReservation));
          ...
          if not zFakeGroup then
            if (isGroupAccount and (FRoomReservation = 0)) or ((not isGroupAccount) and (FRoomReservation = RoomReservation)) then
            begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    isPackage := true;
                    _s := Package_getRoomDescription(Package, room, Arrival, Departure);
                  ...
                  if isPercentage then
                  begin
                    DiscountText := DiscountText + '(' + floattostr(AvrageDiscount) + '%)';
                  end
                  else
                  begin
                    DiscountText := DiscountText + '(' + floattostr(AvrageDiscount) + '%)';
                  ...
                  AddRoom(Room, AvrageRate, Arrival, Departure, UnpaidDays, sText, (FRoomReservation = 0), RoomReservation, AvrageDiscount,
                    // Total discount
                    isPercentage, DiscountText, GuestName, NumGuests, isPackage, RoomReservation);
                end;
                ...
                begin
                  ...
                  AddRoom(Room, 0, Arrival, Departure, UnpaidDays, sText, (FRoomReservation = 0), RoomReservation, 0, false, '', '', NumGuests, isPackage,
                    RoomReservation);
                end;
              end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          sql := 'SELECT * FROM invoicelines' + ' where Reservation = %d ' + '   and RoomReservation = %d ' + '   and SplitNumber = %d ' +
            '   and InvoiceNumber = -1';
          zS := format(sql, [publicReservation, FRoomReservation, FnewSplitNumber]);
          ExecutionPlan.AddQuery(zS);

          sql := 'SELECT * FROM payments ' + ' where Reservation = %d ' + '   and RoomReservation = %d ' + '   and InvoiceNumber = -1';
          ...
          begin
            ...
            CurrencyRate := LocalFloatValue(eSet.FieldByName('CurrencyRate').asString);
            ...
            room := '';
            RoomReservation := eSet.FieldByName('roomreservationAlias').asinteger;
            if mRoomRes.Locate('roomreservation', RoomReservation, []) then
            begin
              room := mRoomRes.FieldByName('room').asString;
            end;
            _s := trim(eSet.FieldByName('Description').asString);
            if (eSet.FieldByName('isPackage').asBoolean) and (pos('Room :', _s) = 0) then
              _s := _s + ' (Room :' + room + ')';

            idx := AddLine(itemId, _s, eSet.GetFloatValue(eSet.FieldByName('Number')), Price, // -96
              Price * eSet.FieldByName('Number').asfloat, // -96
              _DBDateToDate(eSet.FieldByName('PurchaseDate').asString), false, trim(eSet.FieldByName('importRefrence').asString),
              trim(eSet.FieldByName('importSource').asString), eSet.FieldByName('isPackage').asBoolean, eSet.FieldByName('Persons').asinteger,
              eSet.FieldByName('ConfirmDate').asdateTime, eSet.GetFloatValue(eSet.FieldByName('ConfirmAmount')), RoomReservation,eSet.FieldByName('AutoGen').asString,
          ...
          // begin
          // sql := '';
          // sql := sql +'SELECT * FROM invoicelines';
          // sql := sql +' WHERE ';
          //
          // sql := sql +' reservation = '+_db(publicReservation)+' ';
          // sql := sql +' and Splitnumber = '+_db(FnewSplitNumber)+' ';
          // sql := sql +' and isPackage = 1 ';
          //
          // if zrSet.active then zrSet.Close;
          //
          ...
          // begin
          // itemId := trim(zrSet.FieldByName('ItemId').asString);
          //
          // CurrencyRate := LocalFloatValue(zrSet.FieldByName('CurrencyRate').asString);
          // Price := LocalFloatValue(zrSet.FieldByName('Price').asString);
          //
          // if Item_isRoomRent(itemId) then
          // begin
          // if CurrencyRate <> 0 then
          ...
          // end;
          // aRoom := '';
          // tmpint := zrSet.fieldbyname('roomreservation').asinteger;
          // if mRoomRes.locate('roomreservation',tmpInt,[]) = true then
          // begin
          // aRoom := mRoomRes.fieldbyname('room').asstring;
          // end;
          //
          // idx := AddLine(itemId,
          // trim(zrSet.FieldByName('Description').asString+'  Room :'+aroom),
          // zrSet.FieldByName('Number').asfloat, Price, // -96
          // Price * zrSet.FieldByName('Number').asfloat, // -96
          // _DBDateToDate(zrSet.FieldByName('PurchaseDate').asString), false,
          // trim(zrSet.FieldByName('importRefrence').asString),
          // trim(zrSet.FieldByName('importSource').asString),
          // zrSet.FieldByName('isPackage').asBoolean,
          // zrSet.FieldByName('Persons').asinteger,
          // zrSet.FieldByName('ConfirmDate').asdateTime,
          // zrSet.FieldByName('ConfirmAmount').asfloat,
          // zrSet.FieldByName('ItemNumber').asinteger);
          // if (agrLines.Cells[col_Item, agrLines.RowCount - 1] <> '') then inc(iCurrentRow);
          // agrLines.RowCount := iCurrentRow;
          // DisplayLine(iCurrentRow - 1, idx);
          // zrSet.next;
          // end;
        ...
        begin
          d.GetBackupInvoiceLinesReservationRoomReservationsSplitNumber(publicReservation, FRoomReservation, FnewSplitNumber, MemData1);

          MemData1.first;
          while not MemData1.eof do
          begin
            itemId := trim(MemData1.FieldByName('ItemId').asString);

            CurrencyRate := LocalFloatValue(MemData1.FieldByName('CurrencyRate').asString);
            Price := LocalFloatValue(MemData1.FieldByName('Price').asString);
            ...
            idx := AddLine(itemId, trim(MemData1.FieldByName('Description').asString), MemData1.FieldByName('Number').asfloat, Price, // -96
              Price * MemData1.FieldByName('Number').asfloat, // -96
              _DBDateToDate(MemData1.FieldByName('PurchaseDate').asString), false, trim(MemData1.FieldByName('importRefrence').asString),
              trim(MemData1.FieldByName('importSource').asString), MemData1.FieldByName('isPackage').asBoolean, MemData1.FieldByName('Persons').asinteger,
              MemData1.FieldByName('ConfirmDate').asdateTime, MemData1.FieldByName('ConfirmAmount').asfloat, -1, '',
              MemData1.FieldByName('ItemNumber').asinteger);
        ...
        begin
          d.GetBackupPaymentsReservationRoomReservations(publicReservation, FRoomReservation, MemData1);
          ...
          begin
            ...
            begin
              mPayments.Insert;
              mPayments.FieldByName('PayType').asString := MemData1.FieldByName('PayType').asString;
              mPayments.FieldByName('PayDate').asdateTime := _DBDateToDate(MemData1.FieldByName('PayDate').asString);
              mPayments.FieldByName('Amount').asfloat := MemData1.FieldByName('Amount').asfloat;
              mPayments.FieldByName('Description').asString := MemData1.FieldByName('Description').asString;
              mPayments.FieldByName('PayGroup').asString := '';
              // MemData1.FieldByName('').AsString;
              mPayments.FieldByName('Memo').asString := MemData1.FieldByName('Notes').asString;
              mPayments.FieldByName('confirmDate').asdateTime := MemData1.FieldByName('Confirmdate').asdateTime;
              mPayments.FieldByName('Id').asinteger := MemData1.FieldByName('ID').asinteger;

              if glb.Paytypesset.Locate('payType', MemData1.FieldByName('PayType').asString, []) then
              begin
                mPayments.FieldByName('PayGroup').asString := glb.Paytypesset.FieldByName('payGroup').asString;
              end;
              mPayments.post;
              ...
              // idx := AddLine( trim(g.qPaymentItem)
              // , trim(MemData1.FieldByName('Description').asString)
              // , 1
              // , LocalFloatValue(MemData1.FieldByName('Amount').asString) * ( - 1)
              // , LocalFloatValue(MemData1.FieldByName('Amount').asString) * ( - 1)
              // , _DBDateToDate(MemData1.FieldByName('PayDate').asString)
              // , True
              // ,''
              // ,''
              // , False // MemData1.FieldByName('isPackage').asBoolean
              // );
              //
              // if (agrLines.Cells[col_Item, agrLines.RowCount - 1] <> '') then
              // inc(iCurrentRow);
        ...
        inc(iCurrentRow);
        if (agrLines.RowCount > 2) or (trim(agrLines.Cells[col_Item, 1]) <> '') then
        begin
          agrLines.RowCount := iCurrentRow;
...
function TfrmInvoice.LocateDate(recordSet: TRoomerDataset; field: String; Value: TDateTime): boolean;
begin
  result := false;
  recordSet.first;
...
// function TfrmInvoice.calcStayTax2(reservation: integer): boolean;
// var
// taxGuests : integer;
// taxNights : double;
//
...
//
// RemoveStayTax;
// edLodgingTaxISK.Text := '0';
// edLodgingTaxNights.Text := '0';
//
// useStayTax := true;
//
// if useStayTaxAlreadyFetched then
// begin
// useStayTax := useStayTaxOutcome;
// end
// else
// begin
// if ctrlGetBoolean('useStayTax') = false then useStayTax := false;
// if RV_useStayTax(reservation) = false then useStayTax := false;
// useStayTaxAlreadyFetched := true;
...
// end;
//
// if g.qStayTaxPerPerson then
// begin
// StayTaxUnitCount := StayTaxUnitCount+(taxNights*taxGuests);
// end else
// begin
// StayTaxUnitCount := StayTaxUnitCount+taxNights;
// end;
// end;
// end;
// end;
//
//
// if (useStayTax) and (StayTaxUnitCount > 0) then
// begin
// taxPrice := Item_GetPrice(g.qStayTaxItem);
//
// totalTaxISK := StayTaxUnitCount * taxPrice;
//
// edLodgingTaxISK.Text := _FloatToStr(totalTaxISK, vWidth, vDec);
// edLodgingTaxNights.Text := floattostr(StayTaxUnitCount);
//
// if g.qStayTaxPerPerson then
// labTaxNights.Caption := 'Person-nights.'
// else
...
// if taxPrice <> 0 then
// begin
// taxPrice := taxPrice / zCurrencyRate
// end;
//
// if not zstayTaxIncluted then
// begin
// AddRoomTax(taxPrice,StayTaxUnitCount);
// end;
// end;
// DisplayTotals;
// end;
...
/// /  else
/// /  begin
/// /    if ctrlGetBoolean('useStayTax')
/// /      = false then
/// /      useStayTax := false;
/// /    if RV_useStayTax(reservation) = false
/// /    then
/// /      useStayTax := false;
/// /    useStayTaxAlreadyFetched := true;
/// /    useStayTaxOutcome := useStayTax;
...
// roomvat := _calcVAT(totalprice,ItemTypeInfo.VATPercentage);
//
// sTmp := trim(agrLines.Cells[col_NoGuests, i]);
// if sTmp <> '' then
// begin
...
// end;
//
// RoomInvoiceLines.Add(TInvoiceRoomEntity.Create(item, taxGuests, taxNights, totalPrice, roomvat));
// end;
// end;
// end;
//
...
// begin
// ttNative := ttNative+TaxResultInvoiceLines[i].total; // .Amount*TaxResultInvoiceLines[i].NumItems;
// staytaxunitcount := staytaxunitcount + TaxResultInvoiceLines[i].NumItems;
//
//
// if _trimlower(zNativeCurrency) <> _trimlower(zCurrenctCurrency) then
// begin
// if TaxResultInvoiceLines[i].Amount <> 0 then
// TaxResultInvoiceLines[i].Amount := _RoundN(TaxResultInvoiceLines[i].Amount / zCurrencyRate, 2);
// end;
//
// if TaxResultInvoiceLines[i].IncludedInPrice = TIE_PER_CUSTOMER then
// begin
// isIncluted := zstayTaxIncluted;
// end;
//
...
// if isIncluted then
// begin
// ttTaxIncluted := ttTaxIncluted+TaxResultInvoiceLines[i].Amount;
// StayTaxUnitCountIncluted := StayTaxUnitCountIncluted+TaxResultInvoiceLines[i].NumItems;
//
// end else
// begin
// ttTaxExcluted := ttTaxExcluted+TaxResultInvoiceLines[i].Amount;
// StayTaxUnitCountExcluted := StayTaxUnitCountExcluted+TaxResultInvoiceLines[i].NumItems;
// clabLodgingTax.Visible := true;
// end;
// end;
//
//
...
//
//
// clabLodgingTax.Visible := (StayTaxUnitCountIncluted <> 0);
// edLodgingTaxISK.Visible := clabLodgingTax.Visible;
// ClabLodgingTaxCurrency.Visible := clabLodgingTax.Visible;
// labTaxNights.Visible := clabLodgingTax.Visible;
// edLodgingTaxNights.Visible := clabLodgingTax.Visible;
//
/// /    if _trimlower(zNativeCurrency) <> _trimlower(zCurrenctCurrency) then
/// /    begin
/// /      if ttTax <> 0 then
/// /        ttTax := ttTax / zCurrencyRate;
/// /    end;
//
//
// edLodgingTaxISK.Text := _FloatToStr(ttNative, vWidth, vDec);
// edLodgingTaxNights.Text := floattostr(staytaxunitcount);
//
//
//
/// /    edLodgingTaxISK.Text := _FloatToStr((ttTaxIncluted*StayTaxUnitCountIncluted)+ttTaxExcluted*StayTaxUnitCountExcluted, vWidth, vDec);
/// /    edLodgingTaxNights.Text := floattostr(staytaxunitcount);
//
// DisplayTotals;
// finally
...
procedure TfrmInvoice.RemoveTempInvoiceItemList(_tempInvoiceItemList: TList<TInvoiceItemEntity> = nil);
var
  i: integer;
  wasNil : Boolean;
begin
...
function TfrmInvoice.GetItemIvoiceLinesForTaxCalculations: TList<TInvoiceItemEntity>;
var
  taxGuests: integer;
  taxNights: Double;
  //
  ...
  begin
    ...
    begin
      ...
      begin
        // Formula
        // if Item = _trimlower(g.qRoomRentItem) then
        itemVAT := GetVATForItem(Item, itemPrice, 1, TInvoiceRoomEntity.create(Item, taxGuests, taxNights, itemPrice, 0), nil,
        ItemTypeInfo,
        edtCustomer.Text)
        // else
    ...
    begin
      ...
      begin
        ...
        begin
          // Formula
          // if Item = _trimlower(g.qRoomRentItem) then
            itemVAT := GetVATForItem(Item, itemPrice, 1, TInvoiceRoomEntity.create(Item, taxGuests, taxNights, itemPrice, 0), tempInvoiceItemList,
              ItemTypeInfo,
              edtCustomer.Text);
        ...
        if Item = _trimlower(g.qRoomRentItem) then
          RoomInvoiceLines.add(TInvoiceRoomEntity.create(Item, taxGuests, taxNights, itemPrice, itemVAT));

        ItemInvoiceLines.add(TInvoiceItemEntity.create(Item, taxNights, itemPrice, itemVAT));
      end;
    end;

    glb.LocateSpecificRecordAndGetValue('customers', 'Customer', edtCustomer.Text, 'StayTaxIncluted', isIncluted);

    if useStayTax then
      TaxResultInvoiceLines := GetTaxesForInvoice(RoomInvoiceLines, ItemInvoiceLines, isIncluted)
    else
...
//      end;
      ...
      if (StayTaxUnitCountExcluted <> 0) then
      begin
        AddRoomTax(ttTaxExcluted, StayTaxUnitCountExcluted, TaxTypes[l], iAddAt);
      end;
    end;
...
procedure TfrmInvoice.GetTaxTypes(TaxResultInvoiceLines: TList<TInvoiceTaxEntity>);
var
  i: integer;
  Item: String;
begin
...
begin
  ...
  btnRoomToTemp.Enabled := not((publicReservation = 0) AND (FRoomReservation = 0));
...
procedure TfrmInvoice.AddEmptyLine;
begin
  if (agrLines.Cells[col_Item, agrLines.RowCount - 1] <> '') then
    agrLines.RowCount := agrLines.RowCount + 1;
...
begin
  zExit := false;
  agrLines.OnSelectCell := agrLinesSelectCell;
  // caption := 'INVOICE';
...
begin
  // --
  CanSelect := true;
  agrLinesSelectCell(agrLines, 0, 1, CanSelect);
  // agrLines.SetFocus;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        agrLines.Cells[col_ItemPrice, i] := _floattostr(dRoomPrice, vWidth, vDec);
        agrLines.Cells[col_NoGuests, i] := inttostr(iPersons);

        if (agrLines.Objects[2, i] <> nil) and (agrLines.Objects[2, i] is TRoomInfo) then
        begin
          ...
          TRoomInfo(agrLines.Objects[2, i]).FPrice := _StrToFloat(agrLines.Cells[col_ItemPrice, i]);
...
function TfrmInvoice.CheckIfWithdrawlAllowed(removeIfNotAllowed: boolean; Editing: boolean): boolean;
var
  minValue: Double;
begin
  result := true;
  ...
  begin
    if agrLines.Col = col_Item then
      minValue := GridFloatValueFromString(zCellValue) * GridCellfloatValue(agrLines, col_ItemPrice, agrLines.row)
    else if agrLines.Col = col_ItemPrice then
      minValue := GridFloatValueFromString(zCellValue) * GridCellfloatValue(agrLines, col_ItemCount, agrLines.row);
  end;

  if (FRoomReservation > 0) AND (NOT d.roomerMainDataSet.SystemIsRoomWithdrawalAllowed(FRoomReservation,
    (GridCellfloatValue(agrLines, col_ItemCount, agrLines.row) * GridCellfloatValue(agrLines, col_ItemPrice, agrLines.row)) - minValue)) then
  begin
...
begin
  exit;
  // if Key IN [VK_TAB, VK_UP, VK_DOWN, VK_RIGHT, VK_LEFT, VK_SHIFT, VK_LSHIFT,
  // VK_CONTROL, VK_HOME, VK_END, VK_LCONTROL, VK_RCONTROL, VK_F1 .. VK_F24] then
  // exit;
  //
  // if zCellValue <> '' then
  // if agrLines.EditorMode then
  // begin
  // case zCol of
  // 0:
  // begin
  // agrLines.Cells[col_Item, zRow] := zCellValue;
  // agrLines.EditorMode := false;
  // end;
  // end;
  // end;
  //
  // tmp0 := agrLines.Cells[col_Item, zRow];
  // tmp1 := agrLines.Cells[col_Description, zRow];
  // tmp2 := agrLines.Cells[col_ItemCount, zRow];
  // tmp3 := agrLines.Cells[col_ItemPrice, zRow];
  // tmp4 := agrLines.Cells[col_TotalPrice, zRow];
  //
  //
  // // Í sýstemlínu s.s gistingu er bara hægt að breyta lýsingu
  //
  // if (zCol = col_ItemCount) then
  // begin
  // end
  // else
  // begin
  // if zCol <> col_Description then
  // begin
  // if isSystemLine(agrLines.row) then
  // begin
  // showmessage
  // (format(GetTranslatedText('shTx_Invoice_Value_Unchangeable'),
  // [tmp0]));
  //
  // agrLines.Cells[col_Item, zRow] := tmp0;
  // agrLines.Cells[col_ItemCount, zRow] := tmp2;
  // agrLines.Cells[col_ItemPrice, zRow] := tmp3;
  // agrLines.Cells[col_TotalPrice, zRow] := tmp4;
  // Key := vk_Escape;
  // exit;
  // end;
  // end;
  ...
  // // col_confirmAmount = 12;
  //
  // if (Key = vk_Return) or (Key = 37) or (Key = 38) or (Key = 39) or (Key = 40)
  // then
  // begin
  // if agrLines.EditorMode then
  // begin
  ...
  //
  // // ef tómur vörureytur
  // if trim(agrLines.Cells[col_Item, zRow]) = '' then
  // begin
  // // DeleteRow( agrLines, zRow );
  // // AddEmptyLine;
  // // DisplayTotals;
  // end
  // else
  ...
  // MessageDlg(GetTranslatedText('shTx_Invoice_NotAllowed'),
  // mtError, [mbOK], 0);
  //
  // agrLines.Cells[col_Item, zRow] := '';
  // agrLines.Col := 0;
  // agrLines.row := zRow;
  // exit;
  // end;
  ...
  // agrLines.Cells[col_ItemPrice, zRow] := _floattostr(dItemPrice, vWidth, vDec);
  //
  // if NOT CheckIfWithdrawlAllowed(true, false) then
  // exit;
  //
  // if sItemName = '' then
  // begin
  ...
  // _floattostr(1, vWidth, vDec); // -96
  //
  // agrLines.Cells[agrLines.Col + 1, zRow] := sItemName;
  // agrLines.Cells[agrLines.Col + 3, zRow] :=
  // _floattostr(dItemPrice, vWidth, vDec);
  // agrLines.Cells[agrLines.Col + 4, zRow] :=
  // _floattostr(dItemPrice * GridCellfloatValue(agrLines,
  // col_ItemCount, zRow), vWidth, vDec); // -96
  ...
  // zDataChanged := chkChanged;
  //
  // if NOT CheckIfWithdrawlAllowed(false, true) then
  // exit;
  //
  // agrLines.Cells[col_TotalPrice, zRow] :=
  // _floattostr(GridCellfloatValue(agrLines, col_ItemCount, zRow) *
  // GridCellfloatValue(agrLines, 3, zRow), vWidth, vDec); // -96
  // agrLines.Cells[col_System, zRow] := '';
  // end;
  // end;
  // col_ItemPrice:
  ...
  // agrLines.Cells[col_TotalPrice, zRow] :=
  // _floattostr(GridCellfloatValue(agrLines, col_ItemCount, zRow) *
  // GridCellfloatValue(agrLines, col_ItemPrice, zRow), vWidth,
  // vDec); // -96
  // agrLines.Cells[col_System, zRow] := '';
  // agrLines.Cells[col_ItemPrice, zRow] :=
  // _floattostr(GridCellfloatValue(agrLines, 3, zRow),
  // vWidth, vDec);
  // end;
  // end;
  // col_TotalPrice:
  ...
  // _floattostr(GridCellfloatValue(agrLines, col_ItemCount, zRow) *
  // // -96
  // GridCellfloatValue(agrLines, col_ItemPrice, zRow),
  // vWidth, vDec);
  // agrLines.Cells[col_System, zRow] := '';
  // end;
  // end;
...
procedure TfrmInvoice.agrLinesMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: integer);
var
  ACol: integer;
  ARow: integer;
  itemId: string;
...
begin
  ...
  // begin
  // try
  // tmpRoomReservation := TRoomInfo(agrLines.Objects[2, zRow]).FRoomReservation;
  // except
  ...
  // pageMain.ActivePageIndex := 1;
  // end else
  // if (itemId = g.qDiscountItem) then
  // begin
  // pageMain.ActivePageIndex := 1;
  // end;
  // end;
  ...
  agrLines.MouseToCell(X, Y, ACol, ARow);
  if (ACol < 0) OR (ACol >= agrLines.ColCount) OR (ARow < 0) OR (ARow >= agrLines.RowCount) then
    exit;
...
//    exit;
  if { (zCol IN [COL_ITEMCOUNT, COL_ITEMPRICE]) AND }
    (((itemId = g.qRoomRentItem) or (itemId = g.qDiscountItem)) AND (isSystemLine(agrLines.row) = true)) then
  begin
    if tmpRoomReservation = 0 then // þ.e GroupInvoice
    begin
      try
    ...
    if (ACol = col_Description) or (ACol = col_ItemCount) then exit;
...
procedure TfrmInvoice.agrLinesRowChanging(Sender: TObject; OldRow, NewRow: Integer; var Allow: Boolean);
var sCurrentItem,
    sRoomRentItem,
    sDiscountItem : String;
...
begin
  ...
  begin
    ...
    btnRoomToTemp.Enabled := ((sCurrentItem = sRoomRentItem) OR (sCurrentItem = sDiscountItem));
    btnItemToTmp.Enabled := (NOT isSystemLine(NewRow)) AND (sCurrentITem <> '');
...
procedure TfrmInvoice.SaveHeader(FTotal, fVat, fWOVat: Double; ExecutionPlan: TRoomerExecutionPlan);
var
  iMultiplier: integer;
var
  s: string;
...
begin
  ...
  for i := 1 to agrLines.RowCount - 1 do
  begin
    if ((trim(agrLines.Cells[col_Description, i]) = '') and (trim(agrLines.Cells[col_Item, i]) = '')) OR ((isSystemLine(i)) and (iInvoiceNumber <= 0)) then
      continue;
    ...
    if (isSystemLine(i)) or (trim(g.qRoomRentItem) = agrLines.Cells[col_Item, i]) or (trim(g.qDiscountItem) = agrLines.Cells[col_Item, i]) then
    begin
    ...
    tempInvoiceItemList := GetItemIvoiceLinesForTaxCalculations;
    try
      fVat := GetVATForItem(agrLines.Cells[col_Item, i], fItemTotal, _StrToFloat(agrLines.Cells[col_ItemCount, i]),
        TInvoiceRoomEntity.create(agrLines.Cells[col_Item, i], 1, _StrToFloat(agrLines.Cells[col_ItemCount, i]), fItemTotal, 0), tempInvoiceItemList,
        ItemTypeInfo,
        edtCustomer.Text);
    finally
      RemoveTempInvoiceItemList;
    ...
    if (isSystemLine(i)) or (trim(g.qRoomRentItem) = agrLines.Cells[col_Item, i]) or (trim(g.qDiscountItem) = agrLines.Cells[col_Item, i]) then
      ...
      Price := iMultiplier * _CurrencyValueSell * _StrToFloat(agrLines.Cells[col_ItemPrice, i])
    ...
    if (isSystemLine(i)) or (trim(g.qRoomRentItem) = agrLines.Cells[col_Item, i]) or (trim(g.qDiscountItem) = agrLines.Cells[col_Item, i]) then
      // -- Auto-Maintained lines are displayed in foreign currency...
      Total := iMultiplier * _CurrencyValueSell * _StrToFloat(agrLines.Cells[col_TotalPrice, i])
    else // -- ...The others are not...
    ...
    if Assigned(agrLines.Objects[3, i]) AND (agrLines.Objects[3, i] IS TInvoiceLine) then
    begin
...
//          debugmessage(aItem+'/'+booltostr(isSystemLine(i)));
          ...
          if ((isSystemLine(i)) and (aitem<>g.qBreakFastItem)) or (trim(g.qRoomRentItem) = agrLines.Cells[col_Item, i]) or (trim(g.qDiscountItem) = agrLines.Cells[col_Item, i]) then
          begin
            // Setja dagsetningu á herbergisleigu
            // Dagsetning er í upphafi 0  31.12.1899
            // en er hér sett á dagsetningu prentunnar
      ...
      FreeAndNil(ExecutionPlan);
          ...
          begin
            ...
            try
              screen.Cursor := crHourGlass;
              try
                s := 'SELECT * FROM invoicelines' + ' where Reservation = %d ' + '   and RoomReservation = %d ' + '   and SplitNumber = %d ' +
                '   and InvoiceNumber = -1';
...
begin
  ...
  try
    if NOT glb.LocateSpecificRecordAndGetValue('customers', 'Customer', customer, 'StayTaxIncluted', zStayTaxIncluted) then
...
procedure TfrmInvoice.agrLinesSelectCell(Sender: TObject; ACol, ARow: integer; var CanSelect: boolean);
// var
// sItemDescription: string;
// dItemPrice: Double;
begin
  // if zCellEdited then
  // begin
  ...
  // else
  // begin
  // sItemDescription := Item_GetDescription
  // (agrLines.Cells[col_Item, zRow]);
  //
  // dItemPrice := Item_GetPrice(agrLines.Cells[col_Item, zRow]);
  // if NOT d.roomerMainDataSet.OfflineMode then
  // if (GridCellfloatValue(agrLines, col_ItemCount, zRow) = 0) AND
  // (GridCellfloatValue(agrLines, col_ItemPrice, zRow) = 0) AND
  // (FRoomReservation > 0) AND (NOT d.roomerMainDataSet.SystemIsRoomWithdrawalAllowed(FRoomReservation, dItemPrice)) then
  // begin
  // MessageDlg(GetTranslatedText('shUI_AmountOverAllowedWithdrawalLimit'), mtWarning, [mbOk], 0);
  // DeleteRow(agrLines, agrLines.row);
  // AddEmptyLine;
  ...
  // end;
  //
  // if GridCellfloatValue(agrLines, col_ItemCount, zRow) = 0 then // -96
  // agrLines.Cells[col_ItemCount, zRow] :=
  // _floattostr(1, vWidth, vDec);
  //
  // agrLines.Cells[agrLines.Col + 1, zRow] := sItemDescription;
  // agrLines.Cells[agrLines.Col + 3, zRow] :=
  // _floattostr(dItemPrice, vWidth, vDec);
  // agrLines.Cells[agrLines.Col + 4, zRow] :=
  // _floattostr(dItemPrice * GridCellfloatValue(agrLines,
  ...
  // GridCellfloatValue(agrLines, 3, zRow), vWidth, vDec); // -96
  // agrLines.Cells[col_ItemPrice, zRow] :=
  // _floattostr(GridCellfloatValue(agrLines, 3, zRow), vWidth, vDec);
  // // -96
  // agrLines.Cells[col_System, zRow] := '';
  // end;
  // end;
...
procedure TfrmInvoice.agrLinesSetEditText(Sender: TObject; ACol, ARow: integer; const Value: string);
// const
// bAlreadyInSet: boolean = false;
begin
  // if bAlreadyInSet then
  // exit;
  // bAlreadyInSet := true;
  // try
  // zCellEdited := zCellValue <> agrLines.Cells[ACol, ARow];
  // zDataChanged := chkChanged;
  // finally
  // bAlreadyInSet := false;
  // end;
...
procedure TfrmInvoice.agrLinesGetAlignment(Sender: TObject; ARow, ACol: integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
  case ACol of
    col_Item:
...
procedure TfrmInvoice.agrLinesGetCellColor(Sender: TObject; ARow, ACol: integer; AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
begin
  if ARow = 0 then
  begin
...
procedure TfrmInvoice.agrLinesGetEditText(Sender: TObject; ACol, ARow: integer; var Value: string);
begin
  zCellValue := agrLines.Cells[ACol, ARow];
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            MessageDlg(format(GetTranslatedText('shTx_uInvoice_ItemWithIncorrectItemType'), [theData[i].recHolder.Item, ItemType]), mtError, [mbOk], 0);
          end;
        end;
      end;
    end;
...
procedure TfrmInvoice.agrLinesCanEditCell(Sender: TObject; ARow, ACol: integer; var CanEdit: boolean);
begin
  CanEdit := (NOT(ACol IN [col_Item, col_TotalPrice, col_System, col_date, col_Refrence, col_Source, col_isPackage, col_NoGuests, col_confirmdate,
    col_confirmAmount]))
  // AND (
...
begin
  ...
  // if Editing then
  // begin
  // if agrLines.Col = col_ItemCount then
  // minValue := GridFloatValueFromString(zCellValue) *
  // GridCellfloatValue(agrLines, col_ItemPrice, agrLines.row)
  // else
  // if agrLines.Col = col_ItemPrice then
  // minValue := GridFloatValueFromString(zCellValue) *
  // GridCellfloatValue(agrLines, col_ItemCount, agrLines.row);
  // end;

  if (FRoomReservation > 0) AND (NOT d.roomerMainDataSet.SystemIsRoomWithdrawalAllowed(FRoomReservation, currValue - OriginalInvoiceStatus)) then
  begin
    MessageDlg(GetTranslatedText('shUI_AmountOverAllowedWithdrawalLimit'), mtWarning, [mbOk], 0);
    result := false;
  end;
...
begin
  ...
  agrLines.Cells[col_TotalPrice, ARow] := _floattostr(GridCellfloatValue(agrLines, col_ItemPrice, ARow) * GridCellfloatValue(agrLines, col_ItemCount, ARow),
    vWidth, vDec); // -96
end;
...
procedure TfrmInvoice.agrLinesCellValidate(Sender: TObject; ACol, ARow: integer; var Value: string; var Valid: boolean);
var
  sItemName: string;
  dItemPrice: Double;
    ...
    col_Item:
      begin
        ...
        begin
          ...
          if GridCellfloatValue(agrLines, col_ItemCount, zRow) = 0 then
            agrLines.Cells[col_ItemCount, zRow] := _floattostr(1, vWidth, vDec); // -96
...
procedure TfrmInvoice.agrLinesColumnSize(Sender: TObject; ACol: integer; var Allow: boolean);
begin
...
procedure TfrmInvoice.agrLinesDblClickCell(Sender: TObject; ARow, ACol: integer);
begin
...
begin
  ...
  for i := 1 to agrLines.RowCount - 1 do
  begin
    if not edtForeignCurrency.visible or (trim(g.qRoomRentItem) = agrLines.Cells[col_Item, i]) or (trim(g.qDiscountItem) = agrLines.Cells[col_Item, i]) then
    begin
...
begin
  ...
  try
    ...
    begin
      if not SelectPaymentTypes(_StrToFloat(edtBalance.Text), edtCustomer.Text, ptInvoice, lstLocations, zInvoiceDate, zPayDate, zLocation) then
      begin
        exit;
      end;
      ...
      begin
        ...
        raise Exception.create(GetTranslatedText('shTx_Invoice_PaymentTotalInvoice'));
      end;

      if d.roomerMainDataSet.OfflineMode then
      begin
        PrintBackupInvoice(self, TotalPayments, stlPaySelections, days, false);
        ...
        begin
          ...
          theData.AutoGen := _GetCurrentTick;
          theData.TypeIndex := ORD(ptDownPayment);
          theData.invoiceNumber := zInvoiceNumber;
          theData.customer := sSelectedCustomer;
          theData.PayDate := _DateToDbDate(zInvoiceDate, false);
          theData.Amount := _StrToFloat(_strTokenAt(stlPaySelections[i], '|', 1));
          theData.Description := 'Payment Temporary Invoice [' + _strTokenAt(stlPaySelections[i], '|', 0) + ']';
          theData.CurrencyRate := zCurrencyRate; // ATH
          theData.Currency := zCurrenctCurrency;
          theData.confirmDate := 2; // _db('1900-01-01 00:00:00');
          theData.Notes := 'Temporary invoice was printed and paid for during internet outage.';
          theData.PayType := _strTokenAt(stlPaySelections[i], '|', 0);
          theData.AYear := AYear;
          theData.AMon := AMon;
          theData.ADay := ADay;

          AddPaymentToOfflineData(theData);
        end;
        ...
        *)
        ...
        begin
          ...
          begin
            ...
            begin
              // iPersons := TRoomInfo(agrLines.Objects[2, i]).FNumPersons;
              invRoomReservation := TRoomInfo(agrLines.Objects[2, i]).FRoomReservation;
              iNights := trunc(TRoomInfo(agrLines.Objects[2, i]).FTo) - trunc(TRoomInfo(agrLines.Objects[2, i]).FFrom);
            end;
          end;
          ...
          if (LowerCase(trim(g.qRoomRentItem)) = LowerCase(agrLines.Cells[col_Item, i])) OR
            (LowerCase(trim(g.qDiscountItem)) = LowerCase(agrLines.Cells[col_Item, i])) then
            dNumItems := 1.00;
          dLineVAT := GetVATForItem(agrLines.Cells[col_Item, i], dLineTotal, dNumItems, TInvoiceRoomEntity.create(agrLines.Cells[col_Item, i], 1,
          ...
          // if agrLines.Cells[col_ItemCount, i] ='0' then continue;
          ...
          try
             lstActivity.add(CreateInvoiceActivityLog(g.quser
                                   ,publicReservation
                                   ,FRoomReservation
                                   ,FnewSplitNumber
                                   ,ADD_LINE
          ...
          if RoomRentPaid then
            ...
            begin
              ...
              s := s + ' WHERE '#10;
              s := s + ' (Roomreservation = ' + inttostr(invRoomReservation) + ') '#10;;
              s := s + ' AND (Paid = 0) '#10;
        ...
        begin
          ...
          s := s + ', ' + _CommaToDot(trim(floattostr(iMultiplier * _StrToFloat(_strTokenAt(stlPaySelections[i], '|', 1)))));
          s := s + ', ' + _db(PaymentDescription + ' [' + _strTokenAt(stlPaySelections[i], '|', 0) + ']');
          s := s + ', ' + _CommaToDot(floattostr(zCurrencyRate));
          s := s + ', ' + _db(edtCurrency.Text);
        ...
        s := s + '  SET '#10;
        s := s + ' `invoicenumber` = ' + _db(zInvoiceNumber) + ' '#10;
        s := s + ' WHERE (`reservation` = %d) and (`Roomreservation` = %d) and (Invoicenumber=-1) and (typeindex=1); ';
        s := format(s, [publicReservation, FRoomReservation]);
        ExecutionPlan.AddExec(s);
        ...
        begin
          ...
          frmdayNotes.xDoLog(GetTranslatedText('shTx_Invoice_CreatingInvoice'), e.message);
          ...
          // MessageDlg(format(GetTranslatedText('shTx_Invoice_UnableToSaveInvoiceMessage'), [e.message]), mtError, [mbOk], 0);
          if MessageDlg('We are unable to save the invoice. Select [Retry] to try again or [Cancel] ' +#13 + 'to temporarily stop the work with this invoice', mtError, [mbRetry, mbCancel], 0)= mrCancel then
            Close;
    ...
    begin
      ...
      // (MessageDlg('Open a new invoice with the original amounts'+#10+'when finished printing credit invoice ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
        (MessageDlg(GetTranslatedText('shTx_Invoice_OpenInvoiceAfterPrintCredit'), mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
      begin
...
procedure TfrmInvoice.HandleExceptionListFromBookKeepingSystem(invoiceNumber: integer; ErrorList: String);
begin
...
begin
  ...
  if selectedRoomReservation < 0 then exit;

  // if (MessageDlg('Move roomrent to Groupinvoice ' + chr(10) + 'and save other changes ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
  if (MessageDlg(GetTranslatedText('shTx_Invoice_RoomrentToRoomAndSaveChanges'), mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
  begin
    d.UpdateGroupAccountone(publicReservation, selectedRoomReservation, selectedRoomReservation, False);
    SaveInvoice(zInvoiceNumber);
    Close;
  end;
end;
...
procedure TfrmInvoice.agrLinesDrawCell(Sender: TObject; ACol, ARow: integer; Rect: TRect; State: TGridDrawState);
var
  Bmp: TIcon;
  Item: string;
begin
...
begin
  ...
  if zCurrenctCurrency <> oldCurrency then
  begin
    oldRate := GetRate(oldCurrency);
    NewRate := GetRate(zCurrenctCurrency);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          UpdateCurrencyRoomPrice(RoomReservation, zCurrenctCurrency, NewRate, convert);
...
//          mRoomRates.Filtered := true;
          ...
          begin
            ...
            begin
              ...
              d.RR_Upd_CurrencyRoomPrice(RoomReservation, aDate, zCurrenctCurrency, convert);
            end;
            mRoomRates.Next;
          end;
          s := '';
          ...
          s := s + ' SET ' + chr(10);
          s := s + ' Currency=' + _db(zCurrenctCurrency) + ' ' + chr(10);
...
//          mRoomRates.Filtered := false;
    ...
    begin
      UpdateCurrencyRoomPrice(FRoomReservation, zCurrenctCurrency, NewRate, convert);
      mRoomRates.first;
      while not mRoomRates.eof do
      begin
        aDate := _DateToDbDate(mRoomRates.FieldByName('rateDate').asdateTime, false);
        d.RR_Upd_CurrencyRoomPrice(FRoomReservation, aDate, zCurrenctCurrency, convert);
        s := '';
        s := s + ' UPDATE roomreservations ' + chr(10);
        s := s + ' SET ' + chr(10);
        s := s + ' Currency=' + _db(zCurrenctCurrency) + ' ' + chr(10);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          agrLines.Cells[col_ItemPrice, i] := _floattostr(unitPrice, vWidth, vDec);
          agrLines.Cells[col_TotalPrice, i] := _floattostr(totalPrice, vWidth, vDec);
        end;
      end;
    end;
...
var
  ...
  begin
    ...
    if result = -1 then
    begin
      result := stl.add(s + '|' + floattostr(P) + '|' + '0' + '|' + '0' + '|' + '0' + '|' + '0' + '|' + '' + '|');
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      // -- Num Items
      s3 := floattostr(_StrToFloat(s3) + _StrToFloat(agrLines.Cells[col_ItemCount, iRow]));

      // -- Item Total
      s4 := floattostr(_StrToFloat(s4) + (_StrToFloat(agrLines.Cells[col_ItemPrice, iRow]) * _StrToFloat(agrLines.Cells[col_ItemCount, iRow])));
      ...
      begin
        ...
        if agrLines.Objects[2, iRow] <> nil then
          if agrLines.Objects[2, iRow] is TRoomInfo then
            s6 := inttostr(strToIntDef(s6, 0) + TRoomInfo(agrLines.Objects[2, iRow]).FNumPersons * (trunc(TRoomInfo(agrLines.Objects[2, iRow]).FTo) -
              trunc(TRoomInfo(agrLines.Objects[2, iRow]).FFrom)));
      end;
      if s7 = '' then
        s7 := agrLines.Cells[col_Description, iRow];
      stl[i] := s1 + '|' + s2 + '|' + s3 + '|' + s4 + '|' + s5 + '|' + s6 + '|' + s7 + '|';
    end;
  end;
  ...
  begin // STGR med virdisauka
    ...
    for i := agrLines.RowCount - 1 downto 1 do
    begin
      if (agrLines.Cells[col_Item, i] = sSalesItem) or ((sSalesItem = g.qRoomRentItem) and (agrLines.Cells[col_Item, i] = g.qDiscountItem)) or
    ...
    begin
      ...
      agrLines.Cells[col_ItemCount, i + 1] := _floattostr(_StrToFloat(s3), vWidth, vDec); // -96
      agrLines.Cells[col_ItemPrice, i + 1] := _floattostr(_StrToFloat(s2), vWidth, vDec);
      agrLines.Cells[col_TotalPrice, i + 1] := _floattostr(_StrToFloat(s4), vWidth, vDec);
...
begin
  ...
  try
    for i := 1 to agrLines.RowCount - 1 do
    begin
      if (agrLines.Cells[col_Item, i] = sSalesItem) or ((sSalesItem = g.qRoomRentItem) and (agrLines.Cells[col_Item, i] = g.qDiscountItem)
  ...
  begin
    ...
    try
      ...
      *)
      ...
      begin
        zInvoiceNumber := zrSet.FieldByName('InvoiceNumber').asinteger;
        zInvoiceDate := _DBDateToDate(zrSet.FieldByName('InvoiceDate').asString);
        ...
        totalStayTax := LocalFloatValue(zrSet.FieldByName('TotalStayTax').asString);
      ...
      begin
        if not zExit then
        begin
          showmessage(format(GetTranslatedText('shTx_Invoice_ReferenceNumberNotFound'), [zRefNum]));
        end;
        zRefNum := 0;
        zCreditType := ctManual;
        zbDoingReference := false;
      ...
      begin
        ...
        isCurrencyInvoice := trim(UPPERCASE(FieldByName('Currency').asString)) <> trim(UPPERCASE(zNativeCurrency));
        ...
        begin
          ...
          aTotalPrice := aItemPrice * GetFloatValue(FieldByName('Number')); // -96

          idx := AddLine(trim(FieldByName('ItemId').asString), trim(FieldByName('Description').asString), FieldByName('Number').asfloat, aItemPrice,
            aTotalPrice, trunc(now), false, '', '', FieldByName('isPackage').asBoolean, FieldByName('Persons').asinteger, FieldByName('confirmDate').asdateTime,
            GetFloatValue(FieldByName('confirmAmount')), FieldByName('RoomReservationAlias').asinteger,'');
          inc(iCol);
          agrLines.RowCount := iCol;
...
procedure TfrmInvoice.t2Click(Sender: TObject);
begin
  MoveItemToRoomInvoice(SelectableRooms[TMenuItem(Sender).Tag].FRoomReservation);
end;
...
function TfrmInvoice.GetInvoiceHeader(Res, RoomRes: integer; arSet: TdxMemData): boolean;
var
  invoiceNumber: integer;
  customer: string;
  name: string;
...
begin
  ...
  if NOT d.roomerMainDataSet.OfflineMode then
    ...
    rSet := d.roomerMainDataSet.ActivateNewDataset(d.GetBackupReservations(Res));
...
begin
  ...
  rSet := CreateNewDataSet;
  try
    sql := ' SELECT ' + '    Name ' + ' FROM ' + '   persons ' + ' WHERE ' + '   (Reservation = %d) AND (RoomReservation = %d) ';
    s := format(sql, [Res, RoomRes]);
    hData.rSet_bySQL(rSet, s);
    rSet.first;
    if NOT rSet.eof then
  ...
  begin
    ...
    begin
      ...
      try
        ...
        sql := 'SELECT * FROM payments ' + ' where Reservation = %d ' + '   and RoomReservation = %d ' + '   and InvoiceNumber = -1';
        s := format(sql, [publicReservation, FRoomReservation]);
        ...
        begin
          ...
          begin
            ...
            mPayments.FieldByName('PayDate').asdateTime := _DBDateToDate(rSet.FieldByName('PayDate').asString);
...
begin
  ...
  begin
    if MessageDlg('Delete payment ?'#10 + Rec.Description + ' / Amount ' + _floattostr(Rec.Amount, vWidth, vDec), mtConfirmation, [mbYes, mbNo, mbCancel], 0) = mrYes
    then
    begin
      mPayments.delete;
      s := '';
      ...
      begin
        try
          AddInvoiceActivityLog(g.quser
                               ,Rec.reservation
                               ,Rec.RoomReservation
                               ,1    //field typeindex 0 = invoice payment 1 = downpayment
                               ,DELETE_PAYMENT
                               ,Rec.PaymentType
                               ,Rec.Amount
                               ,zInvoiceNumber
...
procedure TfrmInvoice.AddPaymentToOfflineData(theData: recPaymentHolder);
begin
  if d.roomerMainDataSet.OfflineMode then
  begin
    d.GetBackupPaymentsReservationRoomReservations(publicReservation, FRoomReservation, MemData3);

    MemData3.append;

    MemData3['Reservation'] := theData.reservation;
    MemData3['RoomReservation'] := theData.RoomReservation;
    MemData3['InvoiceNumber'] := theData.invoiceNumber;
    MemData3['Person'] := theData.Person;
    MemData3['AutoGen'] := theData.AutoGen;
    MemData3['TypeIndex'] := theData.TypeIndex;
    MemData3['Customer'] := theData.customer;
    MemData3['CurrencyRate'] := theData.CurrencyRate;
    MemData3['Currency'] := theData.Currency;
    MemData3['ReportDate'] := theData.ReportDate;
    MemData3['ReportTime'] := theData.ReportTime;
    MemData3['Description'] := theData.Description;
    MemData3['PayType'] := theData.PayType;
    MemData3['PayDate'] := theData.PayDate;
    MemData3['Amount'] := theData.Amount;
    MemData3['Notes'] := theData.Notes;
    MemData3['AYear'] := theData.AYear;
    MemData3['AMon'] := theData.AMon;
    MemData3['ADay'] := theData.ADay;
    MemData3['pmTmp'] := theData.pmTmp;
    MemData3['confirmDate'] := theData.confirmDate;
    MemData3['ID'] := 0;

    MemData3.post;

    d.BackupPayments(MemData3);
  end;
end;
...
begin
  ...
  begin
    ...
    NewId := 0;
    if d.roomerMainDataSet.OfflineMode OR INS_Payment(theData, NewId) then
    begin
      mPayments.Insert;
      mPayments.FieldByName('PayType').asString := Rec.PaymentType;
...
begin
  ...
  if MessageDlg(format(GetTranslatedText('shTx_Invoice_TakeItemFromInvoice'), [itemId]), mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
  begin
    exit;
  end;
  ...
  tempInvoiceItemList := GetItemIvoiceLinesForTaxCalculations;
  try
    Vat := GetVATForItem(agrLines.Cells[col_Item, CurrentRow], Total, _StrToFloat(agrLines.Cells[col_ItemCount, CurrentRow]),
      TInvoiceRoomEntity.create(agrLines.Cells[col_Item, CurrentRow], 1, _StrToFloat(agrLines.Cells[col_ItemCount, CurrentRow]), Total, 0), tempInvoiceItemList,
      ItemTypeInfo,
      edtCustomer.Text);
  finally
    RemoveTempInvoiceItemList;
  ...
  begin
    ...
    Nights := trunc(TRoomInfo(agrLines.Objects[2, CurrentRow]).FTo) - trunc(TRoomInfo(agrLines.Objects[2, CurrentRow]).FFrom);
  end;
...
begin
  ...
  TotalVAT := GetVATForItem(agrLines.Cells[col_Item, CurrentRow], Total, _StrToFloat(agrLines.Cells[col_ItemCount, CurrentRow]),
    TInvoiceRoomEntity.create(agrLines.Cells[col_Item, CurrentRow], 1, _StrToFloat(agrLines.Cells[col_ItemCount, CurrentRow]), CurrentRow, 0), nil,
    ItemTypeInfo,
    edtCustomer.Text);
  // Formula
  ...
  // s := s + 'á hópreikning ';
  s := s + format(GetTranslatedText('shTx_Invoice_MoveItemToRoomInvoice'), [itemId, Description, RoomByRoomReservation(toRoomReservation)]);
...
//  Description := Description + '(Room:' + RoomNumber + ')';
  ...
  try
    ...
    begin
      rSet.edit;
      rSet.FieldByName('Total').Value := Total + LocalFloatValue(rSet.FieldByName('Total').asString);
      rSet.FieldByName('TotalWOVAT').Value := TotalWOVat + LocalFloatValue(rSet.FieldByName('TotalWOVAT').asString);
      rSet.FieldByName('TotalVAT').Value := TotalVAT + LocalFloatValue(rSet.FieldByName('TotalVAT').asString);
      rSet.post; // ID ADDED
    end;
...
begin
  ...
  // s := s + 'Viltu færa ' + itemId + ': ' + Description + #10;
  // s := s + 'á hópreikning ';
  s := s + format(GetTranslatedText('shTx_Invoice_MoveItemToGroupInvoice'), [itemId, Description]);
...
begin
  ...
  if FRoomReservation > 0 then // 1,234,567,890
    result := strToInt(GetMinutesSinceMidnightAsString + '000000')  + FRoomReservation
  else
    result := strToInt(GetMinutesSinceMidnightAsString + '000000') + publicReservation;
  ...
  try
    ...
    try
      ...
      begin
        ...
        fVat := GetVATForItem(agrLines.Cells[col_Item, i], fItemTotal, dNumItems, TInvoiceRoomEntity.create(agrLines.Cells[col_Item, i], 1,
          _StrToFloat(agrLines.Cells[col_ItemCount, i]), fItemTotal, 0), tempInvoiceItemList,
          ItemTypeInfo,
          edtCustomer.Text);
        if NOT(Item_GetKind(agrLines.Cells[col_Item, i]) IN [ikRoomRent, ikRoomRentDiscount]) then
...
begin
  ...
  result := HeadText + chr(10) + CurrencyText + chr(10) + SumText + chr(10) + GridText;
end;
...
begin
  ...
  if diff then
  begin
    lblChangedInvoiceActive.visible := true;
  end
  else
  begin
    lblChangedInvoiceActive.visible := false;
  end;
end;
...
begin
  ...
  sql := format('SELECT DISTINCT Room, RoomReservation FROM roomsdate WHERE Reservation=%d', [Reservation]);
  rSet := CreateNewDataSet;
  try
