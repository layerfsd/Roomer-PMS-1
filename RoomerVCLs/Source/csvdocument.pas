...
begin
  ...
  repeat
    ...
    begin
      if EndOfLine then
      begin
        FCellBuffer := FCellBuffer + #13#10; // FLineEnding;
        SkipEndOfLine;
      end else
      begin
        FCellBuffer := FCellBuffer + FCurrentChar;
        NextChar;
      end;
    end;
...
begin
  ...
  begin
    FWhitespaceBuffer := FWhitespaceBuffer + FCurrentChar;
    NextChar;
  end;
  // merge whitespace buffer
//  if FIgnoreOuterWhitespace then
//    RemoveTrailingChars(FWhitespaceBuffer, WhitespaceChars);
  FCellBuffer := FCellBuffer + FWhitespaceBuffer;
  FWhitespaceBuffer := '';
end;
...
procedure TCSVParser.SetSource(const AString: String);
begin
  FreeAndNil(FStrStreamWrapper);
