...
var
  i: integer;
begin
  if (Items.ComponentState * [csReading, csDestroying] = []) then
  begin
...
begin
  if (not FIgnoreMenuChanged) then
  begin
    inherited;

    if (GetParentMenu is TTntMainMenu) then
      if (GetParentMenu as TTntMainMenu).FUpdateCount > 0 then
        Exit;
...
procedure TTntMainMenu.BeginUpdate;
begin
  inc(FUpdateCount);
end;

procedure TTntMainMenu.EndUpdate;
begin
...
begin
  ...
  begin
    UpdateMenuItems(Items, Self);
    if (THackMenuItem(Items).FMerged <> nil) then
    begin
