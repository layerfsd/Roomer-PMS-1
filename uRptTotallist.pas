...
{$REGION 'SQL statement'}
      ...
      s := s+'        FROM '#10;
      s := s+'            (SELECT '#10;
      s := s+'                rr.id, rr.ADate '#10;
      s := s+'            FROM '#10;
      s := s+'                roomsdate rr '#10;
      s := s+'            LEFT OUTER JOIN rooms ON (rooms.room = rr.room '#10;
      s := s+'                AND rooms.wildcard = 0) '#10;
      s := s+'            WHERE '#10;
      s := s+'                rr.ResFlag IN '+ cInhouseReservations.AsSQLString;
      s := s+'                    AND ((SUBSTRING(rr.room, 1, 1) = ''<'') '#10;
      s := s+'                    OR (rooms.active = 1))) xx '#10;
      s := s+'        WHERE '#10;
      s := s+'            (xx.ADate = pd.date)) AS roomsInHouse, '#10;
      s := s+'    (SELECT '#10;
      s := s+'            SUM(xx.numGuests) '#10;
      s := s+'        FROM '#10;
      s := s+'            (SELECT '#10;
      s := s+'                (SELECT COUNT(pe.id) FROM persons pe WHERE pe.RoomReservation=rr.RoomReservation) AS numGuests, rr.ADate '#10;
      s := s+'            FROM '#10;
      s := s+'                roomsdate rr '#10;
      s := s+'            LEFT OUTER JOIN rooms ON (rooms.room = rr.room '#10;
      s := s+'                AND rooms.wildcard = 0) '#10;
      ...
      s := s+'                    OR (rooms.active = 1))) xx '#10;
      s := s+'        WHERE '#10;
      s := s+'            (xx.ADate = pd.date)) AS paxinhouse, '#10;
      s := s+'    (SELECT '#10;
      s := s+'            COUNT(xx.id) '#10;
      s := s+'        FROM '#10;
      s := s+'            (SELECT '#10;
