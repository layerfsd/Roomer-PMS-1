...
resourcestring
  ...
  cSQLTest = 'SELECT r.Reservation, r.Name AS ReservationName, ' +
       'rr.RoomReservation, rr.Room, rr.RoomType, rr.Status, '+
       'FORMAT((SELECT AVG(RoomRate) FROM roomsdate WHERE RoomReservation=rr.RoomReservation AND ResFlag=rr.Status), 2) AS AvgRaet, '+
       ...
       'rr.Currency, rr.NumGuests, rr.NumChildren, rr.NumInfants '+
       'FROM reservations r ' +
       'JOIN roomreservations rr ON rr.Reservation=r.Reservation ';

  cSQL = 'SELECT r.Reservation, r.Name AS ReservationName, ' +
       'rr.Arrival, rr.Departure, DATEDIFF(rr.Departure, rr.Arrival) AS Nights, '+
       'rr.RoomReservation, rr.Room, rr.RoomType, rr.Status, '+
       ...
       'p.Person, p.Name AS GuestName, r.ContactName, '+
       'co.NativeCurrency, '+
